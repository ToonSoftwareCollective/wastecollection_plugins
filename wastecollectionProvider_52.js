//<provider>52</provider><version>1.0.0</version><parms>"ICSId"</parms>
//provider defryskemarren.nl testdata: webcal://afvalkalender.defryskemarren.nl/ical/0055200000274723
	...
	function readCalendar(wasteZipcode, wasteHouseNr, extraDates, enableCreateICS, wasteICSId, wasteStreet, wasteStreetName, wasteCity, wasteFullICSUrl) {
				...
				if (xmlhttp.status == 200) {
					...
					if ( i > 0 ) {
						while (i > 0) {
							...
							wasteType = wasteTypeCode(aNode.substring(j+8, j+12));
		...
		xmlhttp.open("GET", "https://afvalkalender.defryskemarren.nl/ical/" + wasteICSId, true);
		xmlhttp.send();
	}
