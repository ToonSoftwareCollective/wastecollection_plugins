//<provider>49</provider><version>1.0.0</version><parms>"zipcode,housenr"</parms>
//provider borssele testdata:https://afvalkalender.borsele.nl/afval/afvalkalender/2022/4437AH-1.ics
	...
	function readCalendar(wasteZipcode, wasteHouseNr, extraDates, enableCreateICS, wasteICSId, wasteStreet, wasteStreetName, wasteCity, wasteFullICSUrl) {
		...
		xmlhttp.open("GET", "https://afvalkalender.borsele.nl/afval/afvalkalender/" + currentYear + "/" + wasteZipcode + "-" + wasteHouseNr + ".ics", true);
		xmlhttp.send();
	}

	function wasteTypeCode(shortName) {
		switch (shortName) {
			case "Groene": return 3;
			case "Grijze": return 0;
